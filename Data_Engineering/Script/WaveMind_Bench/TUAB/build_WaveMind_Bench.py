...
with open(os.path.join(root_dir, 'prompt/WaveMind_Bench/TUAB/Question.txt'), 'r') as f:
...
def generate_qa_dataset(test_dataset, class_all, root_dir, eid_generator,num_options=4):
    ...
    # question_template = f'Please determine if this is a normal EEG? Select one letter in the following. Do not explain. {options_template}'
...
if __name__ == '__main__':
    ...
    for output_mode, dataset_mode in mode_configs:
        ...
        eid_generator=EidGenerator(f"test_{ds_name}_{output_mode}",cache_dir=f'{os.environ["WaveMind_ROOT_PATH_"]}/Data_Engineering/data/WaveMind_Bench/Record_data')
        
        # Load dataset with specified mode
        ds_dataset = CLIPDataset(
            f'{os.environ["WaveMind_ROOT_PATH_"]}/data/Total/data_label.h5',
            mode=dataset_mode,
            ground_truth_dir=f'{os.environ["WaveMind_ROOT_PATH_"]}/data/Total/CLIP_groundTruth',
        ...
        assert len(class_all)==2
        
        # Generate QA dataset
        df_datasets = [generate_qa_dataset(ds_dataset, class_all, root_dir, eid_generator, num_options=2)]

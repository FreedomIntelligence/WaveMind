...
class SubjectEmbedding2(nn.Module):
    def __init__(self, num_subjects, d_model):
        super(SubjectEmbedding2, self).__init__()
        self.subject_embedding = nn.Embedding(num_subjects, d_model)
        self.shared_embedding = torch.randn(1, d_model)  # Shared token for unknown subjects

    def forward(self, subject_ids):

        if subject_ids[0] is None or torch.any(subject_ids >= self.subject_embedding.num_embeddings):
            batch_size = subject_ids.size(0)
            self.update_shared_embedding()
            return self.shared_embedding.expand(batch_size, 1, -1)
        else:
            return self.subject_embedding(subject_ids).unsqueeze(1)

    def update_shared_embedding(self):
        with torch.no_grad():
            self.shared_embedding.data.copy_(self.subject_embedding.weight.data.mean(dim=0, keepdim=True))
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, x_mark, subject_ids=None, mask=None):
        ...
        # Direct value clipping to prevent extreme values while preserving signal characteristics
        # This avoids numerical instability without changing the signal distribution significantly

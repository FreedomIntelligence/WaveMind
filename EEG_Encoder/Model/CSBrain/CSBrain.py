...
if __name__ == '__main__':
    ...
    # Updated parameters for 200Hz input compatibility
    model = CSBrain(in_dim=20, out_dim=200, d_model=200, dim_feedforward=800, seq_len=10, n_layer=6,
                    nhead=8).to(device)
    
    # Test with corrected input shape: (batch, 32, patch_size, num_patches) = (batch, 32, 20, 10)
    a = torch.randn((8, 32, 20, 10)).cuda()
    b = model(a)
    print(f"Input shape: {a.shape}, Output shape: {b.shape}")
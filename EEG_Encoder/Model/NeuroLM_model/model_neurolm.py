...
class NeuroLM(nn.Module):
    ...
    def __init__(self,
        ...
        if tokenizer_ckpt_path is not None:
            ...
            tokenizer_checkpoint = torch.load(tokenizer_ckpt_path,weights_only=False)
            ...
            self.tokenizer.load_state_dict(new_dict,strict=True)
            print('load weight from VQ_align')
        else:
    ...
    def forward(self, x_eeg=None, y_eeg=None, x_text=None, y_text=None, input_chans=None, input_time=None, input_mask=None, eeg_mask=None, eeg_text_mask=None):
        ...
        x_text: shape [B, N2]
        """
        assert x_eeg is not None
        assert x_eeg.shape[-1] == 200

        return self.forward_custom(x_eeg)
    ...
    def forward_custom(self, x_eeg):
        
        # 获取输入数据的维度
        B, N1,T = x_eeg.size()
        x_text=None
        import torchaudio
        # resample from T to 200
        if T != 200:
            resample = torchaudio.transforms.Resample(orig_freq=T, new_freq=200).to(x_eeg.device)
            x_eeg = resample(x_eeg)
        ...
        return {"pooler_output":logits,"last_hidden_state":None}
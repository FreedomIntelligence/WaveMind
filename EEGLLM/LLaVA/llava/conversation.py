...
def get_conversation_template(model_base: str) -> Conversation:
    """Get the conversation template by name."""
    if 'vicuna' in model_base.lower():
        conv = conv_templates['llava_v1'].copy()
    elif 'qwen' in model_base.lower():
        conv = conv_templates['qwen_2_5'].copy()
    elif 'mistral' in model_base.lower():
        conv = conv_templates['mistral_instruct'].copy()
    elif 'llama' in model_base.lower():
        conv = conv_templates['llama_2'].copy()
    else:
        raise ValueError(f"Unknown model base: {model_base}. Please specify a valid conversation template.")
